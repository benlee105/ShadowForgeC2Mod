...
// Function: Get Proxy settings, added in 13 Aug 2025
...
int UpdateTheMessage(const char* message, LPSTR id) {
    ...
    // --- FIX: sanitize the ID before using it ---
    char cleanId[100];  // GUID is max 36 chars + null
    memset(cleanId, 0, sizeof(cleanId));
    strncpy_s(cleanId, sizeof(cleanId), id, _TRUNCATE);

    char* endQuote = strchr(cleanId, '"');
    if (endQuote) {
        *endQuote = '\0';  // terminate at first quote if any JSON junk leaks in
    }

    lstrcatA(endpoint, cleanId);
...
int SendTheMessage(const char* message, LPSTR& id) {
    ...
    // --- FIX: Extract just the GUID from JSON ---
    id = (LPSTR)response;
    id += 7;  // skip '{"id":"'

    char* endQuote = strchr(id, '"');
    if (endQuote) {
        *endQuote = '\0';  // terminate at the first quote
    }
